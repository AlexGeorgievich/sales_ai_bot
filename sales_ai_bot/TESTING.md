...
## 3. Отчеты Allure (с графиками, шагами и видео)

Для генерации интерактивных отчетов с подробными графиками, комментариями к шагам, скриншотами интерфейса и видеозаписями тестов:

1. **Запустите тесты с записью скриншотов, видео и логов в папку результатов**:
   ```bash
   python -m pytest tests/test_frontend.py --alluredir=allure-results --screenshot=on --video=on -v
   ```
2. **Сгенерируйте и откройте отчет в браузере**:
   ```bash
   allure serve allure-results
   ```
   *Это автоматически запустит веб-сервер и откроет дашборд. На вкладке **Suites** вы сможете кликнуть на конкретный тест, увидеть подробные шаги (через `with allure.step`), прикрепленные скриншоты экранов и видеозапись процесса работы браузера.*
